...
export const competitionCategoriesApi = {
  list: (page = 1, search = "") =>
    http.get<PaginatedResponse<CompetitionCategory>>(
      `/competition-categories/?page=${page}&search=${encodeURIComponent(search)}`
    ),

  get: (id: number) =>
    http.get<CompetitionCategory>(`/competition-categories/${id}/`),
